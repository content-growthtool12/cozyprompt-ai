...
export const ShareModal: React.FC<ShareModalProps> = ({ prompt, onClose }) => {
  ...
  const handleCopyLink = () => {
    navigator.clipboard.writeText(`https://cozyprompt.ai/prompt/${prompt.id}`);
    setCopiedLink(true);
    setTimeout(() => setCopiedLink(false), 2000);
  };

  const handleShareTo = (platform: string) => {
    setActionMessage(`Opened sharing hub for ${platform}!`);
  ...
  const platforms = [
    ...
    { name: 'Slack', icon: 'Sparkles', color: 'bg-pink-600/10 border-pink-500/20 text-pink-400 hover:bg-pink-600/20' }
  ];
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-2 p-2 bg-slate-900/60 border border-slate-800 rounded-xl">
            <span className="flex-1 text-[11px] font-mono text-slate-400 truncate px-2 select-all">
              cozyprompt.ai/prompt/{prompt.id}
            </span>
            <button
